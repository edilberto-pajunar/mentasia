...
class FirestoreService {
  ...
  Future<String> uploadProfilePic(file) async {
    ...
    try {
      final image = await StorageService().uploadImageToStorage("profilePics", file);
      await _firebaseFirestore.collection("users").doc(_auth.currentUser!.uid).update({"profilePic": image});
      res = "Success";
    } catch (e) {
      if (kDebugMode) {
        print("Error: $e");
  ...
  Future<void> sendMessage(String message, bool isUserMessage) async {
    try {
      MessageData messageData = MessageData(message: message, isUserMessage: isUserMessage);
  ...
  Stream<QuerySnapshot> readMessage() {
    final documentSnapshot =
        _firebaseFirestore.collection("users").doc(_auth.currentUser!.uid).collection("msg").snapshots();
